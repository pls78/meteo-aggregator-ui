...
- **Desktop (≥ 768px):** search stack top-left; forecast card(s) top-right — cards size to their widest day row (`w-max min-w-72 max-w-[22rem]`), sit side by side when comparing, and scroll within `max-h-[calc(100vh-2rem)]`; layers panel bottom-left (`w-64`); time-lapse pill bottom-center; hourly/confidence sheet slides up bottom-center (`max-w-5xl`, lifted to `bottom-20` when a layer is active so it clears the pill); locate + info controls stacked bottom-right.
...
- **Z-order vocabulary:** map at 0 → in-panel popovers `z-10` → overlay shells `z-[1000]` → hourly sheet / layer scrim `z-[1001]` → layers panel/sheet `z-[1002]` → About dialog `z-[2000]`.
