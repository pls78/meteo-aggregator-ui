...
The satellite-layer list SHALL be derived from the live `GET /imagery` catalog — one entry per
layer the endpoint returns — rather than a hand-maintained copy, so it cannot drift from the
backend. Each layer's update cadence SHALL be derived from the spacing of its returned frame
timestamps. Presentation details not carried by the API (short name, satellite, description)
MAY come from a UI lookup keyed by the stable layer id, and a layer with no such entry SHALL
still be listed using a fallback name and description.
