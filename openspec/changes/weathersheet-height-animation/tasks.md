...
- [ ] 1.1 Give the `WeatherSheet` section a fixed `92vh` layout height and drive
...
- [ ] 1.2 Animate only the transform on snap settle (same 0.3s curve); disable the
      transition under `prefers-reduced-motion: reduce`
- [ ] 1.3 Compensate the internal scroll container with `paddingBottom` equal to the
      off-screen portion so all content is reachable at every snap

## 2. Verify

- [ ] 2.1 `npm run build` + `npm run lint` pass; detector (`detect.mjs --json`) no longer
      reports the animated-layout-property warning
- [ ] 2.2 Manual pass on the dev server (mobile viewport): drag 1:1, tap-cycle, settle
...
- [ ] 3.1 Remove the "WeatherSheet height animation" accepted deviation from `DESIGN.md`
