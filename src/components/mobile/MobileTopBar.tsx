      ...
      <div className="pointer-events-auto flex items-center gap-2">
        {!comparison ? (
          <button
            ...
            onClick={addCompare}
            disabled={!primary}
            className="flex items-center gap-2 rounded-full border border-dashed border-slate-300 bg-white/95 px-3 py-1.5 text-sm font-semibold text-slate-600 shadow-lg disabled:opacity-40"
          >
            <span className="h-2.5 w-2.5 rounded-full" style={{ background: COMPARISON }} />
            + Compare a place
          </button>
        ) : (
          <div className="flex rounded-xl bg-white/95 p-1 shadow-lg ring-1 ring-black/5">
            <button
              type="button"
