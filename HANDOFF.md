...
[`CLAUDE.md`](CLAUDE.md) (architecture + backend contract), which you should read too. This file focuses
...
TypeScript + Tailwind v4 + MapLibre GL) reading from the Python/FastAPI
...
- **Also shipped:** the time-lapse control now stays clear of the open detail sheet
  ...
  sheet overlaps its bottom-left corner (bug fix, direct commit, restoring existing behaviour).
...
work fully now, because all calls are same-origin through `functions/api/` and no
CORS allow-list is involved.
...
5. **Satellite legends** come from the WMS `GetLegendGraphic`. Layers without a real legend
   (e.g. IR 3.9) return a ~20×20 cross-hatched placeholder, so `LayerLegend` hides anything
...
8. **Bundle size:** ~1.3 MB (MapLibre). Vite prints a non-fatal chunk-size warning. Code-split
   if it ever matters.
9. **Bottom-overlay layering (desktop, `App.tsx`).** Three stacked layers share the bottom of the
   ...
   - Known, unfixed: the sheet's `pointer-events-auto` wrapper is full-width, so it still eats map
     clicks in the transparent strips beside the centered sheet. Constraining it needs care,
     because the chart sizes itself from its container width, so don't wrap it in `w-fit`.
10. **`PositionOptions.timeout` does not bound `getCurrentPosition`.** It excludes the time the
    permission prompt is on screen, so an unanswered prompt invokes **neither** callback. That
    ...
    since it's absent on older Safari. Never let it be the thing that decides a request failed.
...
- **Client UI state** → `src/store/appStore.tsx` (Context): `primary`, `comparison`
  ...
  view; `clearDay` leaves it untouched so the panel's close animation doesn't flip content
  mid-fade. Plain click → primary; Shift → comparison. `frameIndex` and
...
2. Author under `openspec/changes/<name>/`: `proposal.md`, `specs/<capability>/spec.md`
   ...
   - A change **must have at least one spec delta**. Even for refactors, frame the user-visible
...
6. `openspec archive <name> --yes` (use `--skip-specs` only for true no-spec changes), which
...
> aggregation section) shipped as direct commits on `main` rather than separate changes; they
> refine content already covered by the `info-page` spec.
